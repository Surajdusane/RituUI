...
export const Index: Record<string, any> = {`
  for (const item of registry.items) {
    const resolveFiles = item.files?.map(
      (file) => `src/registry/ritu/${file.path}`
    )
    if (!resolveFiles) {
      continue
    }
    ...
    const componentPath = item.files?.[0]?.path
      ? `@/registry/ritu/${item.files[0].path}`
      : ""
  ...
  "${item.name}": {
    ...
    files: [${item.files?.map((file) => {
      const filePath = `src/registry/ritu/${typeof file === "string" ? file : file.path}`
  ...
  const fixedRegistry = {
    ...
    items: registry.items.map((item) => {
      const files = item.files?.map((file) => {
        return {
          ...file,
          path: `src/registry/ritu/${file.path}`,
        }
      })
...
async function buildRegistry() {
  return new Promise((resolve, reject) => {
    ...
    const process = exec(
      `bun shadcn build registry.json --output ../../public/r`
    )
...
async function syncRegistry() {
  ...
  await exec("bun run registry:build")
  ...
  // 3. Remove the style directory.
  rimraf.sync(path.join(process.cwd(), "public/r/styles"))

  // 4. Restore the registry content if we had it
  if (registryContent) {
