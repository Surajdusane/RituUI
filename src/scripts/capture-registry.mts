...
const REGISTRY_PATH = path.join(process.cwd(), "public")

// Ensure the thumbs directory exists
const thumbsDir = path.join(REGISTRY_PATH, "thumbs");
...
async function captureScreenshots() {
  ...
  const blocks = blockIds.filter((block) => {
    ...
    const lightPath = path.join(
      REGISTRY_PATH,
      `thumbs/${block}-light.png`
    )
    const darkPath = path.join(
      REGISTRY_PATH,
      `thumbs/${block}-dark.png`
    )
  ...
  for (const block of blocks) {
    ...
    for (const theme of ["light", "dark"]) {
      const screenshotPath = path.join(
        REGISTRY_PATH,
        `thumbs/${block}${theme === "dark" ? "-dark" : "-light"}.png`
      )
